...
public class SATConfigurationEngine extends ConfigurationEngine {
	...
	public SATConfigurationEngine(String featureModelURL, Class<? extends FeatureModel> featureModelClass) throws ConfigurationEngineException {
		...
		try {
    		// model must be satisfiable otherwise raise exception
 	       	FMReasoningWithSAT tempReasoner = new FMReasoningWithSAT("MiniSATHeap", model, 60000);
 	       	tempReasoner.init();
	...
	protected FMReasoningWithSAT createSATReasoner(FeatureModel model) throws Exception {
		FMReasoningWithSAT satReasoner = new FMReasoningWithSAT("MiniSATHeap", model, 60000);
		satReasoner.init();
		return satReasoner;
	}
